# Assignment 08 — Expense Splitter (lite)

> A small group tracks shared expenses — who paid, how much, for whom — and the app computes who owes whom.

**Category:** General-purpose
**Primary user:** A four-person apartment or a trip group.
**Concept this teaches:** Derived balances — never store 'what Alice owes Bob'. Always compute from the expense log. A clean example of single-source-of-truth thinking.
...
1. Is every expense split equally among all members, or can each expense specify which subset it applies to?
2. Does 'who owes whom' settle through a central pool, or as direct pairs? Pairs is simpler but produces n² lines.
3. What does 'settle' mean — a record that zeroes balances up to that point, or just a UI filter?
...
**For this assignment:** Use a real group: name the four people and list three expenses they genuinely had last month.
...
**For this assignment:** Balances are not columns. Write 'derived' in the model for every balance-like concept. If a teammate asks 'where is balance stored', the answer is nowhere.
...
**For this assignment:** Seed expenses such that at least one person owes and one person is owed. If everyone is even, the derivation is not tested.
...
**For this assignment:** The GET /balances endpoint returns the full n × n matrix (or the direct-pair list). Its shape is the most interesting API decision.
...
**For this assignment:** The balance screen is the headline. Make 'you owe' and 'you are owed' visually distinct at a glance.
...
**For this assignment:** Add a new expense and confirm every balance in the UI updates. If any row is stale, you are caching derived state where you should not.
