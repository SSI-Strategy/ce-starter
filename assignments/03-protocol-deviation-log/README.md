# Assignment 03 — Protocol Deviation Log

> A log where monitors record protocol deviations observed at study sites, categorise them, and track each one from open to resolved.

**Category:** Clinical ops
**Primary user:** A clinical monitor responsible for one study across several sites.
**Concept this teaches:** Workflow status as a first-class domain concept: a status column with a CHECK constraint, allowed transitions enforced at the API, and an 'overdue' view computed on the fly.
...
1. Fixed enum (Major / Minor / Other) or free-text taxonomy? For starter scope, pick the fixed enum.
2. What states does a deviation move through — Open, Under review, Resolved, Cancelled? Which transitions are allowed, and from where?
3. What does 'overdue' mean — days since opened without resolution, against what threshold?
...
**For this assignment:** Make the monitor's job real in your head. What does a 'bad week' look like — ten deviations across three sites? Write the stories around that.
...
**For this assignment:** Status is not just a column — it is a state machine. Define the allowed transitions in the model document, not in your head.
...
**For this assignment:** A CHECK constraint on status values is your first line of defence. The API is the second. Without both, drift is inevitable.
...
**For this assignment:** Refuse invalid transitions at the API. A 409 Conflict with a useful message is the right answer — not a silent success.
...
**For this assignment:** The 'overdue' badge is the single most important piece of UI. Make sure the list screen surfaces it on row one.
...
**For this assignment:** Try to move a Resolved deviation back to Open via the UI. If the app lets you, the state machine is leaky.
